    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        """
            ...
            returns:
                training loss
        """
        # TODO: Implement the pseudocode below: do the following ( (DONE)
        ...
        for _ in range(self.num_target_updates):
            target = ptu.from_numpy(reward_n + self.gamma*self.forward_np(next_ob_no)*(1-terminal_n))
            for _ in range(self.num_grad_steps_per_target_update):
                loss = self.loss(target, self(ptu.from_numpy(ob_no)))
