...
class PGAgent(BaseAgent):
    def __init__(self, env, agent_params):
        ...
        # print('\n------Debugging--------')
        # print('in PGAgent',self.agent_params['discrete'])
        # actor/policy
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            # TODO
            # print('\n------Debugging--------')    
            # print('pg_agent,num traj: ',len(rewards_list))
            returns = [self._discounted_return(rs) for rs in rewards_list]

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            # TODO
            returns = [self._discounted_cumsum(rs) for rs in rewards_list]

        q_values = np.hstack(returns)/len(rewards_list) # divide by number of trajectories
        # q_values = np.hstack(returns) 
        return q_values
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            values = unnormalize(values_unnormalized, q_values.mean(), q_values.std())
    ...
    def _discounted_return(self, rewards):
        ...
        """
        R0 = self._discounted_cumsum(rewards)[0]
        list_of_discounted_returns = R0*np.ones(len(rewards))
