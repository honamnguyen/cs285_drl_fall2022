...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        # print('\n------Debugging--------')    
        # print('MLP_policy:',self.forward(observations).log_prob(actions).shape)
        # loss = (-self.forward(observations).log_prob(actions) * advantages).sum()/len(observations)
        ...
        if self.nn_baseline:
            ...
            targets = ptu.from_numpy(normalize(q_values, q_values.mean(), q_values.std()))
            baseline_loss = self.baseline_loss(self.baseline(observations).squeeze(),targets)
