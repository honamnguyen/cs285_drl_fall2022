...
class RL_Trainer(object):
            ...
            if isinstance(self.agent, MBPOAgent):
                for _ in range(self.sac_params['n_iter']):
                    if self.params['mbpo_rollout_length'] > 0:
                        # TODO(Q6): Collect trajectory of length self.params['mbpo_rollout_length'] from the (DONE)
    ...
    def collect_training_trajectories(self, itr, initial_expertdata, collect_policy, num_transitions_to_sample, save_expert_data_to_disk=False):
        ...
        """
        # TODO: get this from previous HW (DONE)
        if itr == 0:
    ...
    def train_sac_agent(self):
        # TODO: Train the SAC component of the MBPO agent. (DONE)
        ...
        for train_step in range(self.sac_params['num_agent_train_steps_per_iter']):
            ob_n, ac_n, re_n, next_ob_n, terminal_n = self.agent.sample_sac(self.sac_params['train_batch_size'])
            self.agent.train_sac(ob_n, ac_n, re_n, next_ob_n, terminal_n)
