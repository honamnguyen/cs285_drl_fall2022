...
class MBPOAgent(BaseAgent):
    ...
    def collect_model_trajectory(self, rollout_length=1):
        # TODO (Q6): Collect a trajectory of rollout_length from the learned (DONE) 
        ...
        for _ in range(rollout_length):
            ...
            for model in self.mb_agent.dyn_models:
                pred_obs.append(model.get_prediction(ob, ac, self.mb_agent.data_statistics))
            pred_obs = np.array(pred_obs)
            next_ob = pred_obs.mean(0)
